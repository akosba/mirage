/*******************************************************************************
 * Author: Ahmed Kosba <ahmed.kosba@alexu.edu.eg>
 *******************************************************************************/

package examples.auxiliary;

import java.math.BigInteger;
import java.util.Arrays;

import jsnark.circuit.structure.CircuitGenerator;
import universal.UniversalCircuitGenerator;
import universal.opcodes.opcode1.OperationType;
import universal.opcodes.opcode2.BitwiseOperationType;
import universal.opcodes.opcode3.Split32OperationUtil;

public class AES128 {

	private int[] inputs;
	private int[] outputs;
	private int[] key;
	private int[] expandedKey;
	
	private UniversalCircuitGenerator generator;

	public static int RCon[] = new int[] { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10,
			0x20, 0x40, 0x80, 0x1b, 0x36 };

	private static int nb = 4;
	private static int nk = 4;
	private static int nr = 6 + nk;
	
	private int galconst;
	
	public static int SBox[] = {  };

	public AES128(UniversalCircuitGenerator generator,
			int[] inputs, int[] key) {
		this.generator = generator;
		this.inputs = inputs;
		this.key = key;
		specifyCircuit();
	}

	public void specifyCircuit() {

		generator.registerMemory("sbox", 256);
		for(int i = 0; i < 256; i++){
			generator.writeToMemory("sbox", generator.makeConstant(i), 
					generator.makeConstant(SBox[i]));
		}
		
		expandedKey = expandKey();
		
		
		galconst = generator.makeConstant(0x1b);
	
		outputs = new int[4 * nb];
		int[][] state = new int[4][nb];
		int i = 0;
		for (int j = 0; j < nb; j++) {
			for (int k = 0; k < 4; k++) {
				state[k][j] = inputs[i++];
			}
		}

		state = addRoundKey(state, 0, nb - 1);
		int round = 0;
		for (round = 1; round < nr; round++) {
			subBytes(state);
			state = shiftRows(state);
			state = mixColumns(state);
			state = addRoundKey(state, round * nb * 4, (round + 1) * nb * 4 - 1);
		}

		subBytes(state);
		state = shiftRows(state);
		state = addRoundKey(state, round * nb * 4, (round + 1) * nb * 4 - 1);

		i = 0;
		for (int j = 0; j < nb; j++) {
			for (int k = 0; k < 4; k++) {
				outputs[i++] = state[k][j];
			}
		}
	}
	
	private int[] expandKey() {
		int[][] w = new int[nb * (nr + 1)][4];
		int[] temp;
		int i = 0;
		while (i < nk) {
			w[i] = new int[] { key[4 * i], key[4 * i + 1], key[4 * i + 2],
					key[4 * i + 3] };
			i++;
		}
		
		i = nk;
		while (i < nb * (nr + 1)) {
			temp = w[i - 1];
			if (i % nk == 0) {
				temp = subWord(generator, rotateWord(generator, temp));
				temp[0] = generator.bitwiseOp(temp[0], generator.makeConstant(RCon[i / nk]), BitwiseOperationType.xor);
			} else if (nk > 6 && (i % nk) == 4) {
				temp = subWord(generator, temp);
			}

			for (int v = 0; v < 4; v++) {
				w[i][v] = generator.bitwiseOp(  w[i - nk][v], temp[v], BitwiseOperationType.xor);
			}
			i++;
		}
		
		int[] expanded = new int[nb * (nr + 1) * 4];
		int idx = 0;
		for (int k = 0; k < nb * (nr + 1); k++) {
			for (i = 0; i < 4; i++) {
				expanded[idx++] = w[k][i];
			}
		}
		return expanded;
	}
	
	
	private  int[] subWord(CircuitGenerator generator, int[] w) {
		for (int i = 0; i < w.length; i++) {
			w[i] = randomAccess(w[i]);
		}
		return w;
	}
	
	private  int randomAccess(int index) {
		return generator.readFromMemory("sbox", index);
	}
	
	
	private  int[] rotateWord(CircuitGenerator generator, int[] w) {
		int[] newW = new int[w.length];
		for (int j = 0; j < w.length; j++) {
			newW[j] = w[(j + 1) % w.length];
		}
		return newW;
	}




	public int[] getOutputs() {
		return outputs;
	}
	
	
	private void subBytes(int[][] state) {
		for (int i = 0; i < state.length; i++) {
			for (int j = 0; j < state[i].length; j++) {
				state[i][j] = randomAccess(state[i][j]);
			}
		}
	}

	private int[][] mixColumns(int[][] state) {

		int a[] = new int[4];
		int c;


		
		for (c = 0; c < 4; c++) {
			for (int i = 0; i < 4; i++) {
				a[i] = state[i][c];
			}
			
			int[] a0_g = galoisMulConst_2_3(a[0]);
			int[] a1_g = galoisMulConst_2_3(a[1]);
			int[] a2_g = galoisMulConst_2_3(a[2]);
			int[] a3_g = galoisMulConst_2_3(a[3]);
			
			int v00 = a0_g[0];
			int v01 = a1_g[1];
			int v02 = a[2];
			int v03 = a[3];
			


			
			int v10 = a2_g[1];
			int v11 = a1_g[0];
			int v12 = a[0];
			int v13 = a[3];
						


			
			int v20 = a2_g[0];
			int v21 = a3_g[1];
			int v22 = a[0];
			int v23 = a[1];



			int v30 = a0_g[1];
			int v31 = a3_g[0];
			int v32 = a[1];
			int v33 = a[2];
			

			
			int packed1 = groupBytes(v00, v10, v20, v30);
			int packed2 = groupBytes(v01, v11, v21, v31);
			int packed3 = groupBytes(v02, v12, v22, v32);
			int packed4 = groupBytes(v03, v13, v23, v33);
			

			int x = xor(xor(packed1, packed2), xor(packed3, packed4));
			int[] bytes = generator.split32ToBytes(x, 32);
			
			state[0][c] = bytes[0];
			state[1][c] = bytes[1];
			state[2][c] = bytes[2];
			state[3][c] = bytes[3];
			
		}
		return state;
	}
	
	private int groupBytes(int b0, int b1, int b2, int b3){
		generator.addBinaryOp(b0, b1);
		generator.setRuntimeSpecEntryForBinaryOp(3, BigInteger.ONE.shiftLeft(8));
		int v2 = generator.customBinaryOp(b2, b3, OperationType.add, OperationType.add, true);
		generator.setRuntimeSpecEntryForBinaryOp(1, BigInteger.ONE.shiftLeft(8*2));
		generator.setRuntimeSpecEntryForBinaryOp(3, BigInteger.ONE.shiftLeft(8*3));
		return v2;
		
	}
	

	private int xor(int id1, int id2){
		return generator.bitwiseOp(id1, id2, BitwiseOperationType.xor);
	}

	
	private int[] galoisMulConst_2_3(int wire) {
		int hiBitSet;
		BigInteger[] coeffs = Split32OperationUtil.getShiftLeftCoefficients(1);
		coeffs[7] = BigInteger.ZERO;
		int[] r = generator.split32ToBitsAndApplySet(wire, 32, coeffs);			
		hiBitSet = r[7];
		int shiftedWire  = r[32];
		int tmp = generator.bitwiseOp(shiftedWire, galconst, BitwiseOperationType.xor);
		int wire1 = generator.muxSelector(shiftedWire, tmp, hiBitSet);
		return new int[]{wire1, generator.bitwiseOp(wire1, wire, BitwiseOperationType.xor)};
	}
	
	private int[][] shiftRows(int[][] state) {
		int[][] newState = new int[4][nb];
		newState[0] = Arrays.copyOf(state[0], nb);
		for (int j = 0; j < nb; j++) {
			newState[1][j] = state[1][(j + 1) % nb];
			newState[2][j] = state[2][(j + 2) % nb];
			newState[3][j] = state[3][(j + 3) % nb];
		}
		return newState;
	}

	private int[][] addRoundKey(int[][] state, int from, int to) {
		int[][] newState = new int[4][nb];
		int idx = 0;
		for (int j = 0; j < nb; j++) {
			for (int i = 0; i < 4; i++) {
//				newState[i][j] = state[i][j].xorBitwise(
//						expandedKey[from + idx], 8);
				newState[i][j] = generator.bitwiseOp(state[i][j], expandedKey[from + idx], BitwiseOperationType.xor);
				idx++;
			}
		}
		return newState;
	}
	

}
