...
{
    // assert(primary_input.size() == num_inputs());
    assert(full_variable_assignment.size() == num_variables());
...
// added by @akosba
template<typename FieldT>
void r1cs_constraint_system<FieldT>::swap_AB_if_beneficial(libff::bit_vector &expensiveG2, libff::bit_vector &freeG2)
{

	// TODO: enhance this method, by keeping track of previous variable assignments.
	// So far, this method implements a very simple heuristic that works fine with our universal circuit shape
    ...
    for (size_t i = 0; i < this->constraints.size(); ++i)
    {
   	...
   	bool to_swap = false;
	int numExpensiveG2inG2 = 0;
	int numExpensiveG2inG1 = 0;
	int numFreeG2inG2 = 0;
	int numFreeG2inG1 = 0;

        for (size_t j = 0; j < this->constraints[i].a.terms.size(); ++j)
        {
		if(expensiveG2[this->constraints[i].a.terms[j].index])
			numExpensiveG2inG1++;
		if(freeG2[this->constraints[i].a.terms[j].index])
			numFreeG2inG1++;
        }
        ...
        for (size_t j = 0; j < this->constraints[i].b.terms.size(); ++j)
        {
		if(expensiveG2[this->constraints[i].b.terms[j].index])
			numExpensiveG2inG2++;
		if(freeG2[this->constraints[i].b.terms[j].index])
			numFreeG2inG2++;           
        }

	if(numExpensiveG2inG2 > numExpensiveG2inG1){
		to_swap = true;
	} else{
		if(numFreeG2inG1 > numFreeG2inG2){
			to_swap = true;
		} 
	}
    ...
    if (!libff::inhibit_profiling_info)
    {
	libff::print_indent();  printf("* \t Made %d swaps.\n", num_swaps);
    }
