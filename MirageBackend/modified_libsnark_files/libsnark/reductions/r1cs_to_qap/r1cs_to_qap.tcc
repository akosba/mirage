...
// akosba: This file has been modified to suit the implementation of MIRAGE
...
// added by @akosba
template<typename FieldT>
qap_instance_evaluation<FieldT> r1cs_to_qap_instance_map_with_evaluation_no_input_constraints(const r1cs_constraint_system<FieldT> &cs,
                                                                         const FieldT &t)
{
    libff::enter_block("Call to r1cs_to_qap_instance_map_with_evaluation");
...
// @akosba: removed the constraints input_i* 0 = 0 as we build on Groth16 -- see note in the modified header file r1cs_to_qap.hpp
/*    for (size_t i = 0; i <= cs.num_inputs(); ++i)
    {
        At[i] = u[cs.num_constraints() + i];
    } */
...
// added by @akosba
template<typename FieldT>
qap_witness<FieldT> r1cs_to_qap_witness_map_no_input_constraints(const r1cs_constraint_system<FieldT> &cs,
                                            ...
                                            const FieldT &d3)
{
    ...
    /* sanity check */
    assert(cs.is_satisfied(full_variable_assignment));
...
// @akosba: removed the constraints input_i* 0 = 0 as we build on Groth16 -- see note in the header file
/*
    for (size_t i = 0; i <= cs.num_inputs(); ++i)
    {
        aA[i+cs.num_constraints()] = (i > 0 ? full_variable_assignment[i-1] : FieldT::one());
